# Hluboká analýza: prepare-proxmox-usb
...
## Popis z README
Děkuji za zpětnou vazbu – vidím, že skript správně detekoval USB disk `sdd` (DataTraveler 80, 57.8 GB) a že jste zadal 
> vme0n1` bez předpony `/dev/`, což skript neakceptoval. Opravil jsem to a zároveň celý skript **výrazně vylepšil** o:

- **Automatické doplnění `/dev/`** – můžete zadat 
> vme0n1`, `sda` i `/dev/sda`
...
|--------|----------------|--------------|
| Zadání disku | Vyžadovalo `/dev/sda` | Lze zadat `sda`, 
> vme0n1` i `/dev/nvme0n1` – skript automaticky doplní `/dev/` |
...
- **Skript běží na vašem aktuálním Proxmoxu** – systémový disk (
> vme0n1`) se **NEMAŽE** hned, přepíše se až při instalaci po restartu. To je bezpečné.
...
## Seznam skriptů
- `infrastructure\prepare-proxmox-usb\prepare-proxmox-usb.sh` – !/bin/bash
- `infrastructure\prepare-proxmox-usb\proxmox-post-install.sh` – !/bin/bash
