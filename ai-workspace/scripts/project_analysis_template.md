...
## Základní informace
- **Cesta v monorepu:** `{TARGET_PATH}`
- **Původní repozitář:** `{REPO_NAME}`
- **Jazyk:** `{PRIMARY_LANGUAGE}`
- **Velikost:** `{SIZE_KB} KB`
- **Licence:** `{LICENSE}`
- **Stav:** `{STATUS}` (aktivní / udržovaný / neudržovaný / archivovaný)
- **Poslední commit:** `{LAST_COMMIT_DATE}`
- **Open issues:** `{OPEN_ISSUES}`
- **Hvězdy:** `{STARS}`, **Forky:** `{FORKS}`
- **Má GitHub Actions:** `{HAS_ACTIONS}`
