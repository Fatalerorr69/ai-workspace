<# Opravny skript pro push pull refs a resume #>
$ErrorActionPreference = 'Stop'

# 1. Uprav migrate_repo.ps1
$migrateContent = @'
param(
...
function Update-State($status) {
    $line = [PSCustomObject]@{
        ...
        wave         = $Wave  # bude doplneno z volajícího
        status       = $status
        timestamp    = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
    }
...
if ($LASTEXITCODE -ne 0) {
    Update-State "clone_failed"
    throw "Klonování selhalo"
}
Set-Location "$workDir\$RepoName.git"

# 3. Použít git filter-repo
...
if (-not $filterRepo) {
    ...
    if ($LASTEXITCODE -ne 0) {
        Update-State "filter_failed"
        throw "git filter-repo selhal"
    }
}

# 4. Push do monorepa (pouze větve a tagy)
if ($TestMode) {
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        # Nevyhazujeme fatální chybu, jen varování
    }
...
'@
Set-Content -Path .\migrate_repo.ps1 -Value $migrateContent

# 2. Uprav batch_migrate.ps1 pro předání Wave a StateFile
$batchContent = @'
param(
...
foreach ($item in $reposToMigrate) {
    ...
    Write-Host "
[$index/$($reposToMigrate.Count)] Zpracovávám $($item.repo_name)" -ForegroundColor Yellow
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        # Nezastavujeme, jen varování
    }
}

Write-Host "
Vlna $Wave – migrace dokončena." -ForegroundColor Green
'@
Set-Content -Path .\batch_migrate.ps1 -Value $batchContent

# 3. Uprav run_migration.ps1 pro Resume a přehled
$runContent = @'
param(
...
if (-not (Get-Command git-filter-repo -ErrorAction SilentlyContinue)) {
    Log "VAROVÁNÍ: git-filter-repo není nainstalován."
}
...
'@
Set-Content -Path .\run_migration.ps1 -Value $runContent

# 4. Vytvoř prázdný state CSV
"repo_name,target_path,wave,status,timestamp" | Set-Content -Path .\migration_state.csv

Write-Host "Oprava dokončena. Nyní spusťte: .\run_migration.ps1 -MonorepoUrl 'https://github.com/Fatalerorr69/ai-workspace.git' -Resume" -ForegroundColor Green
